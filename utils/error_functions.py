...
def compute_classification_error(df_test: pd.DataFrame, expected_labels: dict[Any, Any],
                                 predict: Callable[[pd.Series, Any], Any], model: Any = None) -> int:
...
def compute_classification_error_from_df(df: pd.DataFrame, out_label_col: str, predict: Callable[[pd.Series, Any], Any],
                                         model: Any = None) -> int:
...
def k_fold_cross_validation(df: pd.DataFrame, possible_out_labels: np.ndarray, out_label_col: str,
                            train: Callable[[pd.DataFrame, np.ndarray], Any], predict: Callable[[pd.Series, Any], Any],
                            k: int, random_state: np.random.Generator = None) -> list[dict[str, Any]]:
